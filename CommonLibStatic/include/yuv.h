...
CIYuv<PixelType>::CIYuv(int h, int w, int chroma_format)
{
  ...
  FuncFilter = NULL;
	...
	size_in_byte = (picsizeY + picsizeUV*2)*sizeof(PixelType);   //picture size
...
bool CIYuv<PixelType>::allocate_mem()
{
	...
	memset(&buf1D[picsizeY], 128, picsizeUV*2*sizeof(PixelType));//Owieczka set it  YUV=(0,128,128) mean to black
	...
	for(h=0; h<heightUV; h++, pos+=widthUV)
		U[h] = &(buf1D[pos]);
	for(h=0; h<heightUV; h++, pos+=widthUV)
		V[h] = &(buf1D[pos]);                          //map 1D-buff memeroy to Y U V arrays
...
template <class PixelType>
bool CIYuv<PixelType>::writeOneFrame(FILE *fp, int flag = 0)
{
	if (Y == NULL || fp == NULL || pBuffer == NULL) return false;
	//write YUV format
	...
	case 400:
		for(h=0; h<height; h++)
		{
			...
			for(w=0; w<width; w++)
			{
				...
				Y[h][w] = CLIP3(iy, 0, MaxTypeValue<PixelType>()-1);   //limit the scope of pixel values
			}
		}
		break;
	case 420:
